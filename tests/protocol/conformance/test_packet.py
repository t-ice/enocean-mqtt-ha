from tests.protocol.conformance._timing import timing

from enocean2mqtt.protocol.constants import EVENT_CODE, PACKET, PARSE_RESULT
from enocean2mqtt.protocol.packet import EventPacket, Packet
...
def test_packet_examples():
    ...
    telegram_examples = {
        ...
        PACKET.COMMON_COMMAND: {
            "msg": bytearray(
                [0x55, 0x00, 0x05, 0x00, 0x05, 0xDB, 0x01, 0x00, 0x00, 0x00, 0x0A, 0x54]
            ),
            "data_len": 5,
            "opt_len": 0,
        },
        ...
        PACKET.RESPONSE: {
            "msg": bytearray(
                [0x55, 0x00, 0x05, 0x00, 0x02, 0xCE, 0x00, 0xFF, 0x80, 0x00, 0x00, 0xDA]
            ),
            "data_len": 5,
            "opt_len": 0,
        },
    ...
    for packet, values in telegram_examples.items():
        status, _remainder, pack = Packet.parse_msg(values["msg"])
...
def test_packet_fails():
    ...
    for msg in fail_examples:
        status, _remainder, _packet = Packet.parse_msg(msg)
...
def test_new_common_commands_build_and_roundtrip():
    """The P1 controller commands build valid ESP3 frames with the code as the first data byte."""
    from enocean2mqtt.protocol.constants import COMMON_COMMAND_CODE as CC

    for code in (CC.CO_RD_REPEATER, CC.CO_RD_DUTYCYCLE_LIMIT, CC.CO_WR_IDBASE):
        frame = Packet.create_common_command(code).build()
        status, _rem, packet = Packet.parse_msg(bytearray(frame))
        assert status == PARSE_RESULT.OK
        assert packet.packet_type == PACKET.COMMON_COMMAND
        assert packet.data[0] == int(code)

    # CO_WR_REPEATER carries enable + level as payload bytes.
    frame = Packet.create_common_command(CC.CO_WR_REPEATER, 1, 2).build()
    _status, _rem, packet = Packet.parse_msg(bytearray(frame))
    assert packet.data == [int(CC.CO_WR_REPEATER), 1, 2]
