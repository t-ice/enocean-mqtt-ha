...
from enocean2mqtt.protocol.constants import PACKET, RORG
...
def test_packet_assembly():
    ...
    packet = RadioPacket.create(
        rorg=RORG.BS4, rorg_func=0x20, rorg_type=0x01, learn=True, direction=2, SP=50
    )
    packet_serialized = packet.build()
...
def test_illegal_eep_enum2():
    with pytest.raises(ValueError):
        RadioPacket.create(
            rorg=RORG.RPS, rorg_func=0x02, rorg_type=0x02, sender=[0x00, 0x29, 0x89, 0x79], EB=2
        )
...
def test_vld():
    ...
    packet = RadioPacket.create(
        rorg=RORG.VLD, rorg_func=0x01, rorg_type=0x01, command=1, DV=0, IO=0x1E, OV=0x64
    )
    packet_serialized = packet.build()
